...
import { applyRemember, REMEMBER_COOKIE, type CookieSetOptions } from "@/lib/supabase/cookies";

// Refreshes the Supabase session on every request and bounces anonymous
// visitors to /login. Whether a signed-in user may actually use credentialing
// is a separate question, answered by requireStaff() — middleware runs on the
// edge and has no service-role key, so it cannot read the staff list.
