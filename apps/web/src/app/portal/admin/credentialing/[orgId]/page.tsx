...
import { DeclineEnrollment, NewLocation, NewProvider, OpenEnrollments } from "../forms";
...
export default async function OrganizationPage({
  ...
  const { data: locations } = await db
    .from("location")
    .select("id, name, address_line1, city, state, postal_code, organizational_npi")
  ...
  const [{ data: engagements }, { data: enrollments }, { data: products }, { data: groups }] =
    await Promise.all([
      locationIds.length
        ? db
            .from("engagement")
      ...
      db
        .from("payer_product")
        .select("id, name, credentialing_subject, payer_group:payer_group_id (name)")
        .eq("credentialing_requirement", "required")
        .is("deleted_at", null)
        .order("name"),
      db.from("payer_group").select("id, name").is("deleted_at", null).order("name"),
    ]);

  const locationLabel = (l: Record<string, unknown>) =>
    (l.name as string | null) ?? `${l.address_line1 as string}, ${l.city as string}`;
  ...
  return (
    <div>
      <Link href="/portal/admin/credentialing" className="text-sm text-brand-blue hover:underline">
        ← Credentialing console
      </Link>

      <header className="mt-2 flex flex-wrap items-baseline justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold text-navy-900">{org.legal_name as string}</h2>
      ...
      <section className="mt-8">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Enrollment attaches here. A location bills under its own Type 2 NPI when it has one,
          otherwise the organization&rsquo;s.
        </p>
        {(locations ?? []).length === 0 ? (
          ...
          <ul className="mt-3 divide-y divide-slate-100 overflow-hidden rounded-lg border border-slate-200 bg-white">
            {(locations ?? []).map((l) => {
              ...
              const eng = (engagements ?? []).filter((e) => e.location_id === l.id);
              return (
                <li key={l.id as string} className="px-4 py-3">
                  <div className="flex flex-wrap items-baseline gap-x-3">
                    <span className="font-medium text-navy-900">{locationLabel(l)}</span>
                    <span className="text-sm text-slate-500">
                      {l.city as string}, {l.state as string} {l.postal_code as string}
                    </span>
                  ...
                  {eng.length > 0 && (
                    <ul className="mt-2 flex flex-wrap gap-2">
                      {eng.map((e) => {
                        const p = embedded(e.provider);
                        if (!p) return null;
                        const unknown = p.medicare_enrollment_status === "unknown";
                        return (
                          <li
                            key={e.id as string}
                          ...
                          >
                            ...
                            <span className="text-slate-400">{p.credentials as string}</span>{" "}
                ...
                {(enrollments ?? []).map((e) => (
                  <tr key={e.id as string}>
                    <td className="px-4 py-2.5">
                      <span className="block text-xs text-slate-400">{e.payer_group as string}</span>
                      <span className="font-medium text-navy-900">{e.payer_product as string}</span>
                    </td>
                    <td className="px-4 py-2.5 text-slate-700">{e.provider as string}</td>
                    <td className="px-4 py-2.5 text-slate-500">{e.location as string}</td>
                    <td className="px-4 py-2.5">
