...
export default async function BatchPage({ params }: { params: Promise<{ batchId: string }> }) {
  ...
  const { data: members } = await db
    ...
    .select("id, provider, payer_product, status, effective_date, panel_recheck_due_on")
  ...
  return (
    <div className="max-w-4xl">
      ...
      <h2 className="mt-2 text-xl font-semibold text-navy-900">{payer} submission</h2>
      <p className="mt-1 text-sm text-slate-600">
        {(loc?.name as string | null) ?? (loc?.address_line1 as string)} · {loc?.city as string},{" "}
        {loc?.state as string}
        {batch.reference ? (
          <>
            {" "}
            · ref <span className="font-mono text-xs">{batch.reference as string}</span>
          </>
        ) : null}
      </p>
      {sentTo && sentTo !== payer ? (
        <p className="mt-2 inline-block rounded bg-sky-50 px-2 py-1 text-sm text-sky-800">
          Filed through <strong>{sentTo}</strong> — {payer} delegates its credentialing.
        </p>
      ) : null}
        ...
        <section className="mt-8">
          ...
          <ul className="mt-3 divide-y divide-slate-100 overflow-hidden rounded-lg border border-slate-200 bg-white">
            {(members ?? []).map((m) => {
              ...
              return (
                <li key={m.id as string} className="flex flex-wrap items-baseline gap-x-3 px-4 py-2.5">
                  <span
                    className={`h-2 w-2 shrink-0 rounded-full ${
                      approved ? "bg-emerald-500" : "bg-violet-500"
                    }`}
                  />
                  <span className="font-medium text-navy-900">{m.payer_product as string}</span>
                  <span className="text-sm text-slate-500">{m.provider as string}</span>
