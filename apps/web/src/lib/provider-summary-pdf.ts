...
import type { ProviderSummary } from "@/app/portal/orders/provider-summary";

const NAVY = rgb(0.09, 0.13, 0.24);
const SLATE = rgb(0.42, 0.45, 0.5);
const LINE = rgb(0.85, 0.87, 0.9);

/** Render a branded provider-summary statement PDF → bytes. */
export async function renderProviderSummaryPdf(
  data: ProviderSummary,
  isRep: boolean,
): Promise<Uint8Array> {
  const { provider, orders, totals, periodLabel } = data;
  ...
  const safe = (s: string) =>
    s.replace(/[—–]/g, "-").replace(/…/g, "...").replace(/[•·]/g, "-").replace(/×/g, "x").replace(/[^\x00-\xFF]/g, "");
  ...
  right(`${periodLabel}`, W - M, y - 22, 9, font, SLATE);
  ...
  if (y > M + 30) text("Statement generated from the Agile Medical Group portal.", M, M, 7, font, SLATE);
